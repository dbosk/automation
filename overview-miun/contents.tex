...
\begin{frame}[fragile]\label{WishLateStudents}
...
\subsection{Programmering och Kattis}
...
\begin{frame}[fragile]
  ...
  \begin{example}[Matching]
    \begin{itemize}
      \item datintro\alert<2>{22}\only<3>{, datintro21, datintro20, \dots}
      \item LAB1
    \end{itemize}
  \end{example}
\end{frame}
...
\begin{frame}[fragile]
  ...
  \begin{example}[\texttt{nytid} uses]
    \begin{itemize}
      ...
      \item CLI \mintinline{bash}{nytid} or
            Python \mintinline{python}{import nytid}
    \end{itemize}
  \end{example}
\end{frame}
...
\begin{frame}[fragile]
  \begin{example}[TA pool]
    \begin{itemize}
      ...
      \item \mintinline{bash}{nytid recruit -c "prgi23"}
...
\begin{frame}
  \vspace{-2cm}
  \only<1>{\includegraphics[page=1,width=\textwidth]{figs/weblogin.pdf}}
  \only<2>{\includegraphics[page=3,width=\textwidth]{figs/weblogin.pdf}}
\end{frame}
